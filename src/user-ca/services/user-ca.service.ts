...
import { ADD_NEW_UCA_FAIL, ADD_NEW_UCA_SUCCESS } from 'src/common/constant/message';
...
export class UserCaService {
  ...
  async create(createUserCaDto: CreateUserCaDto) 
  : Promise<FindList<UserCa>> {

    let share = createUserCaDto.share
    ...
    if(share.listIdUsers.length > 0){
      ...
      if(checkUsers.length !== share.listIdUsers.length){
        for(let e of share.listIdUsers){
          let flag = false ;
          for(let element of checkUsers){
            if(e === element.id){
              flag = true
            }
          }
    ...
    for(let e of checkCandidates){
      for(let element of checkUsers){
        ...
        let newUca = await this.ucaEntity.save(infoUca)
        listUca.push(newUca)
      }
    }
