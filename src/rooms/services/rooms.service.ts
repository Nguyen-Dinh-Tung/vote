...
import { USER_CREATE_ROOM_NOT_FOUND, USER_CONNECT_ROOM_NOT_FOUND, ADD_NEW_ROOM_SUCCESS, GET_ROOM_DATA_SUCCESS, ROOM_NOT_FOUND, DUBLICATE_ID_USER, CREATE_GROUP_SUCCESS, GET_GROUP_CHAT_SUCCESS, GET_ROOM_SUCCESS } from './../../common/constant/message';
...
export class RoomsService {
    ...
    async createRoom(res : Response , data : CreateRoomDto){
        ...
        let checkRoomUserInit = await 
        this.roomEntity.createQueryBuilder('rooms')
    ...
    async getRoomData (id : string , res : Response){
        ...
        let data : MessageInterface [] = await 
        this.roomEntity.createQueryBuilder('rooms')
        .leftJoin(ConnectIoEntity , 'cns' , 'cns.roomId = rooms.id')
        .leftJoin(RoomsDataEntity ,'rda' , 'rda.roomId = rooms.id')
        .leftJoin('cns.io' , 'io')
        .leftJoin(UserEntity , 'user' , 'user.ioId = io.id')
    ...
    async createGroupChat (res : Response , data : CreateGroupChatDto){
        ...
        let newRoom = await this.roomEntity.create({
            name : data.name , 
    ...
    async getGroupChats(res : Response , id : string){
        ...
        let groupChats : RoomEntity [] = await 
        this.roomEntity.createQueryBuilder('rooms')
        .leftJoin(ConnectIoEntity , 'cns' , 'cns.roomId = rooms.id')
        .leftJoin('cns.io' , 'io')
        .leftJoin(UserEntity , 'user' , 'user.ioId = io.id')
